...
class InstanceLogPathTests(unittest.TestCase):
    ...
    def test_missing_instance_log_never_uses_another_instance(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            missing = logging_module.get_log_file_path('account_b', root)
            self.assertFalse(missing.exists())
            self.assertEqual(missing.name, f'{datetime.date.today()}_account_b.txt')
            self.assertEqual(logging_module.get_log_file_path('alas', root), default)
            missing.write_text('B 的日志', encoding='utf-8')
            self.assertEqual(logging_module.get_log_file_path('account_b', root).read_text(encoding='utf-8'), 'B 的日志')
