...
class SocketApiTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp.cleanup)
        self.app = create_app(root=fixture(self.temp.name), password='test-secret', manage_runtime=False)
    ...
    def test_clean_rejects_removed_update_methods(self):
        # 即使已登录，旧客户端也不能重新触发外联更新。
        with self.client.websocket_connect('/api/v1/ws') as ws:
            self.login(ws)
            for method in ('updater.status', 'updater.commits', 'updater.fetch',
                           'updater.apply', 'updater.cancel'):
                with self.subTest(method=method):
                    result = self.call(ws, method, {})
                    self.assertEqual('METHOD_NOT_FOUND', result['error']['code'])

    def test_clean_does_not_mount_mcp(self):
        routes = self.client.app.routes
        self.assertFalse(any(getattr(route, 'path', '').startswith('/mcp') for route in routes))
...
class LogCursorTests(unittest.TestCase):
    def test_start_without_updater(self):
