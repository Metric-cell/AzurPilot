...
class DeploySettingsTransactionsTests(unittest.TestCase):
    ...
    def test_runtime_mirror_migration_does_not_reenter_auto_save(self):
        self.file.write_text(TEMPLATE.replace('PypiMirror: null',
                                             'PypiMirror: https://pypi.tuna.tsinghua.edu.cn/simple'))
        config = RuntimeDeployConfig(str(self.file))
        config.Theme = 'dark'
        self.assertEqual(poor_yaml_read(str(self.file))['PypiMirror'],
                         'https://mirrors.aliyun.com/pypi/simple')
        self.assertEqual(config.PypiMirror, 'https://mirrors.aliyun.com/pypi/simple')
...
assert config.WebuiPort == 30001
assert not hasattr(config, 'EnableRemoteAccess')
'''
